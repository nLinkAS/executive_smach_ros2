...
class ContainerProxy():
    ...
    def start(self):
        ...
        self._status_pub_loop()
        self._structure_pub_loop()
    ...
    def _status_pub_loop(self):
        ...
        if self._keep_running and rclpy.ok():
    ...
    def _init_cmd_cb(self, msg):
        ...
        if msg.path == self._path:
            if all(s in self._container.get_children() for s in initial_states):
                ...
                self._server_node.get_logger().debug("Setting initial state in smach path: '"+self._path +
                                                     "' to '"+str(initial_states)+"' with userdata: "+str(ud._data))
                ...
                self._server_node.get_logger().error("Attempting to set initial state in container '"+self._path+"' to '" +
                                                     str(initial_states)+"', but this container only has states: "+str(self._container.get_children()))
...
class IntrospectionServer(Node):
    ...
    def __init__(self, server_name, state, path):
        ...
        Node.__init__(self, server_name, use_global_arguments=False)
